...
class CPlayDice100 {
    ...
    public function PlayDice100() {
        ...
        if (!isset($_GET['setup']) && !$this->players) {
            ...
            if (is_numeric($_GET['noOfPlayers']) && is_numeric($_GET['noOfRobots'])) {
                ...
                for ($pl = 0; $pl < $_GET['noOfPlayers']; $pl++) {
                    $this->players[] = new \CR\CDicePlayer\CDicePlayer($pl + 1, 0);
                }
                for ($pl = 0; $pl < $_GET['noOfRobots']; $pl++) {
                    $this->players[] = new \CR\CDicePlayer\CDicePlayer($counter + 1, 1);
                    $counter++;
                }
            }
        }
        ...
        if ($this->hand) {
            ...
            $this->hand = new \CR\CDiceHand\CDiceHand(1);
            $this->player = $this->players[$this->diceround - 1];
        }
    ...
    private function RunGame() {
        ...
        switch ($this->p) {
            case 'Kasta':
                if ($this->last != 1 && !$this->gameover) { // To prevent manual reload cheat
                    $this->ThrowDice();
                }
                break;
            case 'Spara':
...
EOD;
        ...
        $html .= '<input type="submit" name="p" value="Börja om"></fieldset></form></div>';
